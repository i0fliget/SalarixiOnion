...
export async function manipulateBot(action: string, options: any) {
	...
	if (action === 'recreate') {
		...
		if (result) {
			msg = { type: 'info', success: true, message: `Бот ${options.username} пересоздан` };
		} else {
			msg = { type: 'error', success: false, message: `Не удалось пересоздать бота ${options.username}` };
		}
	} else if (action === 'message') {
		if (flow[options.username]?.bot) {
			flow[options.username]!.bot?.chat(options.message);
			msg = { type: 'info', success: true, message: `Бот ${options.username} отправил сообщение "${options.message}" в чат` };
		} else {
			msg = { type: 'error', success: false, message: `Бот ${options.username} не смог отправить сообщение` };
		}
	}
	...
	try {
		...
		bot.once('spawn', async () => {
			try {
				...
				await sleep(randint('float', 1000, 2000));
	...
	try {
		...
		if (method === 'register') {
			if (!profile.registered) {
				...
				await sleep(randint('float', options.registerMinDelay, options.registerMaxDelay));
			...
			if (profile.registered) {
				...
				await sleep(randint('float', options.loginMinDelay, options.loginMaxDelay));
...
async function recreateBot(username: string) {
	try {
		...
		await sleep(randint('float', 1000, 2800));
...
function cleanupBot(username: string) {
  const profile = flow[username]?.profile;
  if (profile?.intervals) {
    profile.intervals.forEach(i => clearInterval(i));
...
process.on('uncaughtException', (e) => {
  ...
  const seconds = date.getSeconds().toString().padStart(2, '0');
	...
	console.log(`[ ${hours}:${minutes}:${seconds} ~ Ошибка ]: ${e}`);
});