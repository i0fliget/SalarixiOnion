import { randstr, randint, randelem } from './generator.js';
...
export function mutateText(options: any) {
  ...
  text = text.replace(/\[(.+)\]/g, (_: any, value: string) => randelem(value.split(';')));
  ...
  text = text.replace(/#([nlms])\((\d+)-(\d+)\)/g, (match: any, type: string, min: string, max: string) => {
    ...
    switch (type) {
      case 'n':
        result = randstr('numeric', randint('int', Number(min), Number(max))); break;
      case 'l':
        result = randstr('letter', randint('int', Number(min), Number(max))); break;
      case 'm':
        result = randstr('multi', randint('int', Number(min), Number(max))); break;
      case 's':
        result = randstr('special', randint('int', Number(min), Number(max))); break;
      default: 
        break;
    }
  ...
  text = text
    .replace(/#n/g, () => randstr('numeric', 3))
    .replace(/#l/g, () => randstr('letter', 3))
    .replace(/#m/g, () => randstr('multi', 3))
    .replace(/#s/g, () => randstr('special', 3));

  if (options.advanced) {
    text = text.replace(/#p/g, () => randelem(options.data.players));
  }
